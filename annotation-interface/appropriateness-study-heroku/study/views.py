...
def upload_posts(request):
    ...
    if request.method != 'GET':
        ...
        if not csv_file.name.endswith('.csv'):
            messages.error(
                request, "Please upload a .csv file with columns : 'id', 'source', 'rewrite_a', 'rewrite_b', 'issue', 'batch', 'edits_a', 'edits_b' ")
        else:
            try:
                data_set = csv_file.read().decode('UTF-8')
                io_string = io.StringIO(data_set)

                # Use DictReader to properly handle quoted fields with commas
                reader = csv.DictReader(io_string)

                for row in reader:
                    # Parse JSON edits if present
                    edits_a = json.loads(row.get('edits_a', '[]')) if row.get('edits_a') else []
                    edits_b = json.loads(row.get('edits_b', '[]')) if row.get('edits_b') else []

                    post, created = models.StudyPost.objects.update_or_create(
                        id=row['id'],
                        defaults={
                            'source': row['source'],
                            'rewrite_a': row['rewrite_a'],
                            'rewrite_b': row['rewrite_b'],
                            'issue': row['issue'],
                            'batch': row['batch'],
                            'edits_a': edits_a,
                            'edits_b': edits_b
                ...
                messages.success(request, f"Successfully uploaded {reader.line_num - 1} posts")

            except KeyError as e:
                messages.error(request, f"CSV missing required column: {e}")
            except json.JSONDecodeError as e:
                messages.error(request, f"Error parsing JSON in edits: {e}")
            except Exception as e:
                messages.error(request, f"Error uploading posts: {e}")
