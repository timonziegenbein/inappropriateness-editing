...
def get_post_info(post_id):
    ...
    # Add edits fields if they exist in the CSV
    if 'edits_a' in df.columns:
        data['edits_a'] = df['edits_a'].values[0]
    else:
        data['edits_a'] = '[]'

    if 'edits_b' in df.columns:
        data['edits_b'] = df['edits_b'].values[0]
    else:
        data['edits_b'] = '[]'
