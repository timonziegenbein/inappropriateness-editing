...
def get_post_info(post_id):
    ...
    data['source'] = get_value(df, 'source')
    data['inappropriate_part'] = get_value(df, 'inappropriate_part')
    data['rewritten_part'] = get_value(df, 'rewritten_part')
    data['issue'] = get_value(df, 'issue')
...
def get_annotations_info(user_id, batch):
    ...
    # Randomize order for each user (consistent per user)
    post_df = post_df.sample(frac=1, random_state=user_id).reset_index(drop=True)
    ...
    # Add a summary of the edit for display in the dashboard
    post_df['rewrite'] = post_df.apply(
        lambda row: f"{row['inappropriate_part']} → {row['rewritten_part']}"
        if pd.notna(row['inappropriate_part']) and pd.notna(row['rewritten_part'])
        else '',
        axis=1
    )
