...
def upload_posts(request):
    ...
    if request.method != 'GET':
        ...
        if not csv_file.name.endswith('.csv'):
            messages.error(
                request, "Please upload a .csv file with columns : 'id', 'source', 'inappropriate_part', 'rewritten_part', 'issue', 'batch' ")
        else:
            ...
            for column in csv.reader(io_string, delimiter=','):
                post, created = models.StudyPost.objects.update_or_create(
                    ...
                    defaults={
                        'source': column[1],
                        'inappropriate_part': column[2],
                        'rewritten_part': column[3],
                        'issue': column[4],
                        'batch': column[5]
