"""
Interactive test script for the Global Pattern Conformity Scorer.

This script allows you to test the global pattern conformity scorer with example arguments
...
Usage:
    ...
    python scorers/test_global_pattern_conformity_scorer.py

    # Directly specify a file to cycle through
    python scorers/test_global_pattern_conformity_scorer.py --input_jsonl models/generated_edits/grpo_global_sentence_v11.jsonl

    # Start from a specific example index
    python scorers/test_global_pattern_conformity_scorer.py --input_jsonl <path> --example_index 5

    # Limit number of examples to show
    python scorers/test_global_pattern_conformity_scorer.py --input_jsonl <path> --max_examples 10

    # Use predefined example
    python scorers/test_global_pattern_conformity_scorer.py --predefined 0

    # Use custom model path and threshold
    python scorers/test_global_pattern_conformity_scorer.py --input_jsonl <path> --model_path <path> --threshold 2.5
"""
...
def visualize_tokenization(scorer: GlobalPatternConformityScorer, text: str):
...
def visualize_edit_mapping(scorer: GlobalPatternConformityScorer, text: str, tokens: List[str],
...
def visualize_edit_sequence(scorer: GlobalPatternConformityScorer, text: str, edits: List[Dict]):
...
def visualize_perplexity_calculation(scorer: GlobalPatternConformityScorer, sequence: List[str]):
    """Visualize the perplexity calculation."""
    print_subsection("Step 3: Calculate Perplexity")

    # Show vocab mapping
    print("Vocabulary mapping:")
    from scorers.global_scorers.pattern_conformity.global_pattern_conformity_scorer import pc_vocab
    for token, idx in sorted(pc_vocab.items(), key=lambda x: x[1]):
        print(f"  {token:15s} -> {idx}")

    # Convert to integers
    sequence_as_int = [pc_vocab.get(token, 0) for token in sequence]
...
def test_global_pattern_conformity_scorer(argument: str, edits: List[Dict],
                                  model_path: str = None, threshold: float = None):
    """Test the global pattern conformity scorer with detailed output."""
    print_section("Global Pattern Conformity Scorer Test")
    ...
    scorer = GlobalPatternConformityScorer(**scorer_kwargs)
    ...
    binary_score, actual_perplexity = scorer.calculate_global_pattern_conformity(argument, edits)
    ...
    if binary_score == 1.0:
        ...
        print(f"\n{Colors.GREEN}✓ The edit pattern shows PATTERN CONFORMITY at the document level{Colors.ENDC}")
    else:
        print(f"Binary Score: {Colors.RED}{Colors.BOLD}{binary_score:.1f} (FAIL){Colors.ENDC}")
        print(f"\n{Colors.RED}✗ The edit pattern does NOT show PATTERN CONFORMITY at the document level{Colors.ENDC}")
...
def get_predefined_examples():
    ...
    # Example 1: Few edits (should be pattern conformity)
    examples.append({
        "name": "Few edits (likely pattern conformity)",
    ...
    # Example 2: Many edits (likely not pattern conformity)
    examples.append({
        "name": "Many edits (likely not pattern conformity)",
    ...
    with open(filepath, 'r', encoding='utf-8') as f:
        for i, line in enumerate(f):
            ...
            try:
                ...
                test_global_pattern_conformity_scorer(argument, edits, model_path, threshold)
...
def main():
    parser = argparse.ArgumentParser(description="Interactive test for Global Pattern Conformity Scorer")
    ...
    if args.predefined is not None:
        ...
        if 0 <= args.predefined < len(examples):
            ...
            test_global_pattern_conformity_scorer(example['argument'], example['edits'], args.model_path, args.threshold)
        else:
        ...
        print(f"{Colors.BOLD}{Colors.HEADER}Global Pattern Conformity Scorer Interactive Test{Colors.ENDC}\n")
        filepath = interactive_file_selection()
        if filepath:
            cycle_through_file(filepath, args.model_path, args.threshold, args.example_index, args.max_examples)
