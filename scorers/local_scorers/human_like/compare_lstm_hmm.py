#!/usr/bin/env python3
"""
Compare LSTM and HMM components for human-like edit scoring.
...
def compute_lstm_scores(model, sequences: List[List[int]], max_length: int, device: str = 'cuda') -> np.ndarray:
    ...
    Returns:
        perplexities: Array of perplexity scores (lower = more human-like)
    """
    model.eval()
    ...
    # Return raw perplexities (lower = more human-like)
...
def main():
    ...
    # Paths
    lstm_model_path = Path("scorers/local_scorers/human_like/models/lstm_model.pt")
    # Use the hmm_only model (pure HMM without Isolation Forest)
    hmm_model_path = Path("scorers/local_scorers/human_like/models/hmm_only_hmm_model.pkl")
    test_data_path = Path("scorers/local_scorers/human_like/data/test_sequences.pkl")
    ...
    if not hmm_model_path.exists():
        ...
        logger.error("Example: python scorers/local_scorers/human_like/compute_hmm_isolation_scores.py --hmm-only --save-models --model-prefix hmm_only_")
        return
