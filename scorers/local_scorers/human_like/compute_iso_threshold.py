...
The ISO model outputs scores where higher = more human-like (less anomalous).
We compute Q1 - 1.5×IQR as the lower bound threshold.
Scores >= threshold are considered human-like.
"""
...
def compute_threshold(scores: List[float]) -> tuple:
    ...
    iqr = q3 - q1
    threshold = q1 - 1.5 * iqr  # Lower bound (lower score = less human-like)
...
def main():
    ...
    print("\nInterpretation:")
    print(f"  - ISO score >= {threshold:.4f} → human-like edit")
    print(f"  - ISO score < {threshold:.4f} → not human-like")
    print(f"  - Higher score = more human-like")
    ...
    print("\nNext steps:")
    print("  1. Update human_like_scorer.py with this threshold")
    if args.per_sentence:
