...
from scorers.local_scorers.pattern_conformity.model_defs import LanguageModel

logger = logging.getLogger(__name__)

# V2 vocab with 'keep-in-edit' token
pc_vocab = {'<pad>': 0, 'keep': 1, 'del': 2, 'add': 3, 'replace': 4, 'keep-in-edit': 5}

class PatternConformityScorer:
    def __init__(self, device, model_path="scorers/local_scorers/pattern_conformity/models/pattern_conformity_v4.pth",
                 threshold=2.452725653648376, max_len=500):
        """
        Initialize PatternConformityScorer.

        Args:
            device: torch device
            model_path: Path to the trained model checkpoint
            threshold: Perplexity threshold for pattern conformity classification (default: 2.4527, P99 from v4)
        ...
        self.model, self.tokenizer = self._load_pc_model(device)
        logger.info(f"Loaded PatternConformityScorer: model={model_path}, threshold={threshold}")

    def _load_pc_model(self, device):
        """Loads the pattern conformity model and tokenizer."""
        pc_embedding_dim = 200
        pc_nhead = 2
        pc_nhid = 200
        pc_nlayers = 2
        vocab_size = len(pc_vocab)

        # Create model with dropout=0 for inference (no dropout during evaluation)
        pattern_conformity_model = LanguageModel(
            vocab_size, pc_embedding_dim, pc_nhead, pc_nhid, pc_nlayers, dropout=0.0
        ).to(device)

        # Load model checkpoint
        checkpoint = torch.load(self.model_path, map_location=device)
        if isinstance(checkpoint, dict) and 'model_state_dict' in checkpoint:
            pattern_conformity_model.load_state_dict(checkpoint['model_state_dict'])
        else:
            pattern_conformity_model.load_state_dict(checkpoint)

        pattern_conformity_model.eval()
        pattern_conformity_tokenizer = AutoTokenizer.from_pretrained("meta-llama/Llama-3.1-8B-Instruct")
        return pattern_conformity_model, pattern_conformity_tokenizer
    ...
    def _calculate_perplexity_for_sequence(self, sequence):
        ...
        padded_input = np.array(input_seq[:self.max_len] + [pc_vocab['<pad>']]*(self.max_len - len(input_seq)) if len(input_seq) < self.max_len else input_seq[:self.max_len])
        padded_target = np.array(target_seq[:self.max_len] + [pc_vocab['<pad>']]*(self.max_len - len(target_seq)) if len(target_seq) < self.max_len else target_seq[:self.max_len])
        ...
        criterion = nn.CrossEntropyLoss(ignore_index=pc_vocab['<pad>'])

        with torch.no_grad():
            output = self.model(inputs)
            logger.debug(f"  Output tensor shape: {output.shape}")
            loss = criterion(output.view(-1, len(pc_vocab)), targets.view(-1))
    ...
    @weave.op()
    def calculate_pattern_conformity(self, original_argument, original_sentence, inappropriate_part, rewritten_part):
        """
        Calculates the pattern conformity reward for an edit.
        """
        ...
        pattern_conformity_reward = 0.0
        if sequence:
            perplexity = self._calculate_perplexity_for_sequence(sequence)
            if perplexity <= self.threshold:
                pattern_conformity_reward = 1.0
        return pattern_conformity_reward
