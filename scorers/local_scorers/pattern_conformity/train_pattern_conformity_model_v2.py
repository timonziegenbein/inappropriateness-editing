...
from scorers.local_scorers.pattern_conformity.model_defs import LanguageModel, EditSequenceDataset
...
def main():
    ...
    parser.add_argument('--wandb-project', type=str, default='pattern-conformity-scorer',
    ...
    if args.input_csv:
        ...
        temp_csv = '/tmp/pattern_conformity_sequences_train.csv'
        df = pd.DataFrame({
    ...
    artifact = wandb.Artifact(
        name=f"pattern-conformity-model-v2",
        type="model",
        description="Pattern conformity edit scorer language model v2 with keep-in-edit token"
    )
