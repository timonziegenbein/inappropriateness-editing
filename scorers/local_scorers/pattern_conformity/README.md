# Pattern Conformity Scorer

This directory contains the ISO+HMM (Isolation Forest + Hidden Markov Model) pattern conformity scorer for evaluating edit pattern conformity in the appropriateness editing task.

## Overview

The pattern conformity scorer uses anomaly detection to identify whether an edit follows human editing patterns. It combines:
...
scorer = PatternConformityScorer(device="cuda")
score = scorer.calculate_pattern_conformity(
...
# Returns 1.0 if pattern conformity (score >= 0.232), else 0.0
```
...
pattern_conformity/
├── README.md                          # This file
├── pattern_conformity_scorer.py               # Main scorer implementation
...
scorer = PatternConformityScorer(device='cuda' if torch.cuda.is_available() else 'cpu')
score = scorer.calculate_pattern_conformity(
