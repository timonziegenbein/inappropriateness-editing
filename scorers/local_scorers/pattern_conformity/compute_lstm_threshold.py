...
def compute_threshold(perplexities):
    ...
    - Perplexity <= threshold → pattern conformity
    - Perplexity > threshold → not pattern conformity
    ...
    # Upper bound threshold (higher perplexity = less pattern conformity)
    threshold = q3 + 1.5 * iqr
...
def main():
    ...
    if args.per_sentence:
        model_path = Path("scorers/local_scorers/pattern_conformity/models/lstm_model_per_sentence.pt")
        data_path = Path("scorers/local_scorers/pattern_conformity/data/train_sequences_per_sentence.pkl")
        output_file = Path("scorers/local_scorers/pattern_conformity/models/lstm_threshold_per_sentence.txt")
        mode = "strategy-level (per-sentence)"
    else:
        model_path = Path("scorers/local_scorers/pattern_conformity/models/lstm_model.pt")
        data_path = Path("scorers/local_scorers/pattern_conformity/data/train_sequences.pkl")
        output_file = Path("scorers/local_scorers/pattern_conformity/models/lstm_threshold_edit.txt")
        mode = "edit-level"
    ...
    print(f"\nInterpretation:")
    print(f"  - Perplexity <= {threshold:.4f} → pattern conformity edit")
    print(f"  - Perplexity > {threshold:.4f} → not pattern conformity")
    print(f"  - Lower perplexity = more predictable = more pattern conformity")
    ...
    print(f"\nNext steps:")
    print(f"  1. Update pattern_conformity_scorer.py with this threshold")
    if args.per_sentence:
