"""
Generate edits using OpenAI's GPT models to create a baseline for comparison.

This script uses GPT models to generate edits for the appropriateness editing task,
producing output compatible with evaluate_edits.py for direct comparison with
GRPO-trained models.

Usage:
    python models/generate_edits_openai.py \
        --output_jsonl models/generated_edits/gpt4o_baseline.jsonl \
        --model_name gpt-4o \
        --split validation \
        --concurrency 10
"""
...
class OpenAIEditGenerator:
    """
    Generate text edits using OpenAI's GPT models.
    """

    def __init__(self, model_name: str = "gpt-4o", timeout: int = 120):
        """
        Initialize the OpenAI edit generator.
...
Always respond with valid JSON in the exact format specified."""
    ...
    async def generate_edits(self, issue: str, sentences: str, max_retries: int = 3) -> Dict[str, Any]:
        """
        Generate edits using the OpenAI API.
        ...
        for attempt in range(max_retries):
            try:
                ...
                response = await self.client.chat.completions.create(
                    ...
                    top_p=1.0,  # Greedy decoding (deterministic)
                )
                ...
                # Log full message details if content is None or empty
                if completion is None or len(completion.strip()) == 0:
                    logger.warning(f"Empty completion from API!")
                    logger.warning(f"  Model: {response.model}")
                    logger.warning(f"  Finish reason: {response.choices[0].finish_reason}")
                    logger.warning(f"  Message role: {message.role}")
                    logger.warning(f"  Message content: {message.content}")
                    if hasattr(message, 'refusal') and message.refusal:
                        logger.warning(f"  Refusal: {message.refusal}")
                    completion = "" if completion is None else completion.strip()
                else:
                    completion = completion.strip()

                # Log if completion is very short
                if len(completion) < 50 and len(completion) > 0:
                    logger.warning(f"Very short completion: '{completion}' (length: {len(completion)})")
...
async def process_example(
    generator: OpenAIEditGenerator,
    example: Dict[str, Any],
    idx: int
) -> Dict[str, Any]:
    """Process a single example through OpenAI."""
...
async def generate_all_edits_async(
    ...
    generator = OpenAIEditGenerator(model_name=model_name, timeout=timeout)
...
def main(
    ...
    """Generate edits using OpenAI and save them to a JSONL file."""
    os.makedirs(os.path.dirname(output_jsonl), exist_ok=True)

    logger.info(f"Starting OpenAI edit generation on {split} split. Output file: {output_jsonl}")
    ...
    print(f"    --output_jsonl models/predictions/gpt4o_baseline.jsonl")


if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Generate edits using OpenAI GPT models as a baseline.")
    ...
    print("=" * 80)
    print("GENERATING EDITS USING OPENAI GPT MODELS (BASELINE)")
    print("=" * 80)
    print(f"\nConfiguration:")
