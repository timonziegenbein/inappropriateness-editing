...
# Import DebertaPredictor only when needed (to avoid dependency issues)
DebertaPredictor = None
...
class MetricsCalculator:
    def __init__(self,
        ...
        if classifier_prediction:
            global DebertaPredictor
            if DebertaPredictor is None:
    ...
    def set_classifier_based_on_fold(self, fold):
        ...
        model_id = "timonziegenbein/appropriateness-classifier-binary"

        self.classifier = DebertaPredictor(
            model_id, self.tokenizer, self.batch_size, device=self.device, device_map=self.device_map)
    ...
    def calculate_metrics(self, x, y):
        ...
        if self.perplexity:
            ...
            gt_perplexities = self.perplexity_model._compute(
                data=y, batch_size=1)['perplexities']
            prompt_perplexities = self.perplexity_model._compute(
                data=x, batch_size=1)['perplexities']
